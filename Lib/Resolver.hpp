// Made by @IkuzaDev [https://github.com/IkuzaDev/UnityResolver] modified by @batchh
...
public:
	...
	struct Class final {
		...
		auto Get(const std::string& name, const std::vector<std::string>& args = {}) -> RType* {
			if constexpr (std::is_same_v<RType, Field>) {
				for (auto pField : fields) if (pField->name == name) return static_cast<RType*>(pField);
			}
			if constexpr (std::is_same_v<RType, std::int32_t>) {
				for (const auto pField : fields) if (pField->name == name) return reinterpret_cast<RType*>(pField->offset);
			}
					...
					if (pMethod->name == name) {
						...
						if (pMethod->args.size() == args.size()) {
							...
							for (size_t i = 0; i < args.size(); i++) {
								const auto& typeName = args[i];
								if (typeName == "*" || typeName.empty() ? true : pMethod->args[i]->pType->name == typeName) index++;
							}
	...
	struct Field final {
		...
		template <typename T>
		auto SetStaticValue(const T& value) const -> typename std::enable_if<!std::is_pointer<T>::value, void>::type {
			if (!static_field) return;
			if (mode_ == Mode::Il2Cpp) {
				Invoke<void, void*, const T*>("il2cpp_field_static_set_value", address, &value);
			} else {
				void* VTable = Invoke<void*>("mono_class_vtable", pDomain, klass->address);
				Invoke<void, void*, void*, const T*>("mono_field_static_set_value", VTable, address, &value);
			}
		}

		template <typename T>
		auto SetStaticValue(T value) const -> typename std::enable_if<std::is_pointer<T>::value, void>::type {
			if (!static_field) return;
			if (mode_ == Mode::Il2Cpp) {
				Invoke<void, void*, T*>("il2cpp_field_static_set_value", address, &value);
			} else {
				void* VTable = Invoke<void*>("mono_class_vtable", pDomain, klass->address);
				Invoke<void, void*, void*, T*>("mono_field_static_set_value", VTable, address, &value);
			}
		}
		...
		template <typename T>
		auto GetStaticValue() const -> typename std::enable_if<std::is_pointer<T>::value, T>::type {
			if (!static_field) return nullptr;
...
#if WINDOWS_MODE || LINUX_MODE || __APPLE__ /*__cplusplus >= 202002L*/
	static auto DumpToFile(const std::string path) -> void {
		...
		for (const auto& pAssembly : assembly) {
			io << "\tnamespace: " << (pAssembly->name.empty() ? "" : pAssembly->name);
			io << "\n";
			io << "\tAssembly: " << (pAssembly->name.empty() ? "" : pAssembly->name) << "\n";
			io << "\tAssemblyFile: " << (pAssembly->file.empty() ? "" : pAssembly->file) << " \n";

			for (const auto& pClass : pAssembly->classes) {
				io << "\t" << pClass->getType() << " " << pClass->name << (pClass->parent.empty() ? "" : " : " + pClass->parent);

				for (const auto& pField : pClass->fields) {
					std::stringstream fieldStr;
					fieldStr << "\t\t" << std::hex << std::showbase << std::internal 
							<< pField->offset << " | " << (pField->static_field ? "static " : "") 
				...
				methodStr << "\t\t[Flags: " << std::bitset<32>(pClass->flags).to_string() 
						 << "] [ParamsCount: " << pClass->methods[0]->args.size() 
						 << "] |RVA: " << std::hex << std::showbase 
						 << (reinterpret_cast<std::uint64_t>(pClass->methods[0]->function) - 
							 reinterpret_cast<std::uint64_t>(hmodule_)) << "|\n";
				io << methodStr.str();

				std::stringstream paramStr;
				paramStr << "\t\t" << pClass->methods[0]->return_type->name << " " << pClass->name << "(";
				std::string params{};
				for (const auto& pArg : pClass->methods[0]->args) {
					paramStr << pArg->pType->name << " " << pArg->name << ", ";
				}
				std::string paramString = paramStr.str();
				if (!paramString.empty()) {
					paramString = paramString.substr(0, paramString.length() - 2);
				}
				io << paramString << ");\n";

				io << "\t\t//" << pClass->methods[0]->return_type->name << " " << pClass->name << "(System.Void)\n\n";
			}
	...
	public:
		...
		struct Quaternion {
			...
			auto Euler(float m_fX, float m_fY, float m_fZ) -> Quaternion {
				...
				const auto m_fSinX = std::sin(m_fX);
				const auto m_fCosX = std::cos(m_fX);

				const auto m_fSinY = std::sin(m_fY);
				const auto m_fCosY = std::cos(m_fY);

				const auto m_fSinZ = std::sin(m_fZ);
				const auto m_fCosZ = std::cos(m_fZ);
